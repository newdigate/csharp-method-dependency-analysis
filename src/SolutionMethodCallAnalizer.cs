...
public class SolutionInterfaceMethodAnalysis {
    private readonly IClassDependencyAnalyzer _classDependencyAnalyzer;
    public SolutionInterfaceMethodAnalysis(IClassDependencyAnalyzer classDependencyAnalyzer)
    {
        _classDependencyAnalyzer = classDependencyAnalyzer;
    }
    
    public IDictionary<ISymbol, IDictionary<ISymbol, IList<ISymbol>>> AnalizeMethodCallsForSolution(string solutionPath) {
        EnsureMsBuildRegistration();
        using (MSBuildWorkspace workspace = MSBuildWorkspace.Create())
        {
            workspace.WorkspaceFailed += (sender, workspaceFailedArgs) => Console.WriteLine(workspaceFailedArgs.Diagnostic.Message);
            IDictionary<ISymbol, IDictionary<ISymbol, IList<ISymbol>>> result = new Dictionary<ISymbol, IDictionary<ISymbol, IList<ISymbol>>>();

            Solution solution = workspace.OpenSolutionAsync(solutionPath).Result;
            foreach (ProjectId projectId in workspace.CurrentSolution.GetProjectDependencyGraph().GetTopologicallySortedProjects()) {
                Project? project = workspace.CurrentSolution.GetProject(projectId);
                if (project == null) continue;
                IDictionary<ISymbol, IDictionary<ISymbol, IList<ISymbol>>> substitutions =  AnalizeMethodCallsForProject(project);
                result = result.Union(substitutions).ToDictionary(k => k.Key, v => v.Value);
            }
    ...
    public IDictionary<ISymbol, IDictionary<ISymbol, IList<ISymbol>>> AnalizeMethodCallsForProject(string projectPath) {
        EnsureMsBuildRegistration();
        using(MSBuildWorkspace workspace = MSBuildWorkspace.Create()) 
        {
            workspace.WorkspaceFailed += (sender, workspaceFailedArgs) => Console.WriteLine(workspaceFailedArgs.Diagnostic.Message);
    ...
    public IDictionary<ISymbol, IDictionary<ISymbol, IList<ISymbol>>> AnalizeMethodCallsForProject(Project project) {
        EnsureMsBuildRegistration();
        Compilation? compilation = project.GetCompilationAsync().Result;
               
        if (compilation == null) {
            IDictionary<ISymbol, IDictionary<ISymbol, IList<ISymbol>>> empty = new Dictionary<ISymbol, IDictionary<ISymbol, IList<ISymbol>>>();
            return empty;
        }
        ...
        return
            _classDependencyAnalyzer
                .AnalizeClassCalls(compilation, roots);
    }
